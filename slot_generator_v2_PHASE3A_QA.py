...
# ============================================================
# R&B FITNESS – SLOT GENERATOR (PHASE 2B/2C/2D + PHASE 3A)
# WhatsApp-safe output: NO blank lines, uses "—" separators.
#
# Modes:
# 1) Interactive (default): prompts date/time/names
# 2) Auto stub mode: reads bookings_stub.json, generates sessions in next window
#    python slot_generator_v2.py --auto --window-hours X
#
# PHASE 3A (A) – Conditioning Creativity Upgrade:
# - Controlled randomness with WEIGHTED rotation by "conditioning format"
# - Tries to avoid visible conditioning pattern repetition across clients in same slot
# - Tries to avoid repeating same conditioning format for same client vs last session
# - Still respects:
#   * injury bans
#   * equipment reality
#   * machine congestion
...
#
# PATCH (03/04/26):
# - Auto-fix if Claude forgets the "Conditioning" header but outputs the allowed conditioning block.
#
# PATCH (UNILATERAL RULE - STEP A TIGHTEN):
# - Only TRUE unilateral/isolation (1 arm / single arm / single leg / split squat / step ups / kickbacks etc.)
#   may use 10L/10R to 12L/12R.
# - If Claude puts L/R on a bilateral lift (Prone Row DB etc.), system auto-fixes it to 10 to 12.
#
# PATCH (STEP A TIGHTEN):
# - Hard block +X3/+X4/+X5 anywhere in client blocks.
# - Remove "STOP" if model outputs it.
# - If model fails validation after retries -> system FALLBACK BUILDER outputs a valid client block
#   (no more ERROR blocks in generated files).
# ============================================================
...
# =========================
# NOTE: Unilateral reps are allowed ONLY for true single-side work.
# Bilateral movements must ALWAYS stay 10 to 12 even if model outputs L/R.
UNILATERAL_KEYWORDS = [
    "single arm",
    "1 arm",
    "one arm",
...
# If Claude ever outputs L/R on any of these, we force it back to 10 to 12.
# (These are clearly bilateral patterns we see commonly.)
...
def enforce_rep_scheme_in_output(text: str) -> str:
    """
    STEP A:
    - If unilateral exercise: force 10L/10R to 12L/12R
    - If bilateral exercise: force 10 to 12 (remove any L/R)
    Applies to rehab + main work + core finisher.
    """
    if not text:
        return text
    ...
    for raw in lines_in:
        ...
        if is_unilateral_name(name_only):
            ...
            # Force bilateral reps (remove any L/R patterns)
            fixed = s
            if has_lr:
            ...
            # If reps missing entirely, do nothing here (validator catches missing reps)
            out.append(fixed.strip())
...
def _is_overloaded_error(exc: Exception) -> bool:
    ...
    if "overloaded" in name:
        return True
    msg = str(exc).lower()
    return ("overloaded" in msg) or ("error code: 529" in msg) or (" 529 " in msg) or ("status code: 529" in msg)
...
def _looks_like_transient_error(exc: Exception) -> bool:
    ...
    transient_class_hints = [
        "ratelimit", "rate_limit",
        "timeout", "timedout",
        "apiconnection", "connection",
        "serviceunavailable", "unavailable",
        "internalserver", "servererror", "apierror",
        "temporarily", "retry",
    ]
    if any(h in name for h in transient_class_hints):
        return True
    ...
    if any(h in msg for h in transient_msg_hints):
        return True
...
GLOBAL_BANNED_EXERCISES = {
    ...
    "Barbell Hip Thrust",  # STEP A: recommended ban for perfect-first-time
}
...
# ======= CLIENT LISTS (CONFIG OVERRIDES SUPPORTED) =======
...
ENFORCE_UNIQUE_CONDITIONING_FORMAT_PER_SLOT = bool(_cfg_get("enforce_unique_conditioning_format_per_slot", ENFORCE_UNIQUE_CONDITIONING_FORMAT_PER_SLOT))
ALLOW_CONDITIONING_FORMAT_RELAX_FALLBACK = bool(_cfg_get("allow_conditioning_format_relax_fallback", ALLOW_CONDITIONING_FORMAT_RELAX_FALLBACK))
...
R&B FITNESS – SLOT GENERATOR (PHASE 2B/2C/2C/2D ADVANCED)
...
def validate_client_block(output_text: str, expected_client: str, expected_focus_label: str, flags: dict,
    ...
    if not spice_mode:
        ...
        for sup in range(3):
            ...
            if not (("10 to 12" in norm(ex_a)) or ("10l/10r" in norm(ex_a))):
                return False, f"Superset {sup+1} exercise A missing reps."
            if not (("10 to 12" in norm(ex_b)) or ("10l/10r" in norm(ex_b))):
                return False, f"Superset {sup+1} exercise B missing reps."
...
def build_fallback_standard_block(client_name: str, focus_label: str, main_bank: list, abs_bank: list,
    ...
    """
    Outputs a guaranteed-valid STANDARD block (3 supersets + conditioning OR core finisher).
    Enforces:
    - Fly + Pushdown not paired
    - At least 1 abs exercise
    - Sets lines are +X2 to 4 by default
    """
    no_cardio = bool(flags.get("force_core_finisher") or flags.get("force_no_cardio"))
    ...
    abs_norm = norm(abs_name) if abs_name else ""

    # Build 3 supersets (6 exercises)
    # Ensure at least 1 abs is included in the 6 (prefer slot 6 if possible)
    pairs = []

    # Try to pick 5 non-abs main exercises then abs as last if available
    for _ in range(5):
        nm = _pick_main_exercise(main_bank, rng, avoid_norm=used, must_not_include=set())
        if not nm:
            break
        used.add(norm(nm))
        pairs.append(nm)
    ...
    # If still short, fill with remaining main exercises
    while len(pairs) < 6:
        nm = _pick_main_exercise(main_bank, rng, avoid_norm=used, must_not_include=set())
        if not nm:
            break
    ...
    # If not enough, bail to error
    if len(pairs) < 6:
        return "\n".join([f"{client_name}: {focus_label}", "—", "ERROR: NEED MORE EXERCISES IN BANK", "—"])

    # Assemble into 3 supersets with rule enforcement
    idx = 0
    for sup in range(3):
        a = pairs[idx]
        b = pairs[idx + 1]
        ...
        if (_is_fly_name_raw(a) and _is_pushdown_name_raw(b)) or (_is_fly_name_raw(b) and _is_pushdown_name_raw(a)):
            ...
            for k in range(idx, len(pairs)):
                ...
                break
            if not swapped:
                # last resort: just force error to be safe
                return "\n".join([f"{client_name}: {focus_label}", "—", "ERROR: NEED MORE EXERCISES IN BANK", "—"])

        rep_a = unilateral_rep_text() if is_unilateral_name(a) else "10 to 12"
        rep_b = unilateral_rep_text() if is_unilateral_name(b) else "10 to 12"
    ...
    if omit_cond:
        # No conditioning or core finisher at all
        return "\n".join(lines).strip()

    if no_cardio:
        # Core finisher: pick 2 abs/core if possible
        lines.append("Core Finisher")
        a2 = abs_name or _pick_abs_exercise(abs_bank, rng)
        b2 = _pick_abs_exercise(abs_bank, rng)
        if not a2 or not b2:
            # fallback to any main if abs bank too small
        ...
        rep_a2 = unilateral_rep_text() if is_unilateral_name(a2) else "10 to 12"
        rep_b2 = unilateral_rep_text() if is_unilateral_name(b2) else "10 to 12"
...
def generate_client_block_with_retries(system_prompt: str, user_prompt: str, expected_client: str, expected_focus_label: str,
    ...
    # STEP A: fallback instead of error block
...
def _iso_to_ddmmyy(dt: datetime) -> str:
...
def generate_slot_output(slot_date_ddmmyy: str, slot_time_display: str, raw_client_inputs: list, slot_focus: str,
    ...
    slot_clients = []
    missing = []
    for nm in normalised_inputs:
        c = match_client_strict(nm, clients_dict)
        if not c:
            missing.append(nm)
        else:
    ...
    if missing:
        lines = [
            "ERROR: These names do NOT match any Excel sheet tab (after alias normalisation):",
            "—",
        ]
        lines.extend([f"- {m}" for m in missing])
        lines.extend([
            "—",
            "Fix: type the exact sheet name (e.g., Stuart G, Owen M, Anne W, Kirsty M, Anjali).",
            "—",
        ])
        return "\n".join(lines), log
    ...
    for idx, client in enumerate(slot_clients):
        ...
        main_bank, abs_bank_original = apply_injury_bans(
            main_bank, abs_bank_original, client.get("injuries", ""), client_name=real_name
        )
        ...
        for ex in main_bank:
            ...
            if not nm:
                continue
            if norm(nm) in slot_used_exercise_names:
                continue
            if used_leg_ext_curl_combo and nm in ("Leg Curl Machine", "Leg Extension Machine", "Hamstring Curl Machine"):
                continue
            if used_cable_pull_through and nm == "Cable Pull Through":
                continue
        ...
        try:
            ...
            note = "Claude API overloaded/transient error. Re-run for this slot later." if _looks_like_transient_error(e) else "Unexpected error."
            # If Claude call fails entirely, still output fallback (perfect-first-time behaviour)
        ...
        if ("ERROR:" not in block) and block.strip() != "ERROR: NEED MORE EXERCISES IN BANK":
            for ex_line in extract_exercise_lines(block):
                nm2 = norm(extract_name_from_output_line(ex_line))
                if nm2 and nm2 in abs_names_lookup:
                    used_abs_norm.add(nm2)
...
def _ensure_inputs_exist_or_print_and_exit():
...
def run_interactive():
    ...
    clients = load_clients(os.path.join(os.getcwd(), EXCEL_FILENAME))
    log = load_log()
...
def run_auto_from_stub(window_hours: int = 24):
    _ensure_inputs_exist_or_print_and_exit()
    ...
    for dt, s in sorted(due, key=lambda x: x[0]):
        ...
        try:
            ...
            err_lines = [
                ...
                slot_time_display,
                "—",
                ", ".join([safe_str(x) for x in clients_list]),
                "—",
            ...
            if _looks_like_transient_error(e):
                err_lines.insert(6, "NOTE: Claude API overloaded/transient. Re-run auto later for this slot.")
            with open(out_path, "w", encoding="utf-8") as f:
                f.write("\n".join(err_lines).strip() + "\n")
            print(f"Generated (ERROR): {AUTO_OUTPUT_DIR}\\{fname}")
            continue
...
def main():
    parser = argparse.ArgumentParser(description="R&B Fitness Slot Generator (Interactive + Auto Stub Mode)")
    parser.add_argument("--auto", action="store_true", help="Auto-run sessions from bookings_stub.json in next window.")
    parser.add_argument("--window-hours", type=int, default=24, help="Auto window size in hours (default 24).")
    parser.add_argument("--qa-all", action="store_true", help="QA: generate + validate 1-client slots for ALL clients and save a PASS/FAIL report.")
    parser.add_argument("--qa-date", type=str, default=datetime.now().strftime("%d/%m/%y"), help="QA date in dd/mm/yy (default today).")
    parser.add_argument("--qa-focus", type=str, default="back and biceps", help="QA focus used for all clients (default: back and biceps).")
    parser.add_argument("--qa-limit", type=int, default=0, help="QA: limit number of clients (0 = all).")
    ...
    if args.auto:
        run_auto_from_stub(window_hours=args.window_hours)
        return
